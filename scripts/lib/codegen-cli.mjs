...
export function buildGoOperations(ops) {
  ...
  for (const op of ops) {
    ...
    if (op.pagination) {
      ...
      lines.push(`\tif opts.Page > 0 { query.Set("page", fmt.Sprintf("%d", opts.Page)) }`);
      lines.push(`\tif opts.PageSize > 0 { query.Set("pageSize", fmt.Sprintf("%d", opts.PageSize)) }`);
    }
    ...
    lines.push(`\tif err := c.Do(ctx, req, &out); err != nil { return nil, err }`);
