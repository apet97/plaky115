...
- A new file under `sdk/src/runtime/` must be declared: add it to
  `publicRuntimeModules` in `scripts/lib/sdk-runtime-modules.mjs` and run
  `generate:all`, or to `intentionallyPrivateRuntimeModules` in
  `scripts/audit-package-artifacts.mjs`. `npm run artifacts:audit` fails on an
  undeclared module.
